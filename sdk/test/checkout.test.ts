...
import { EtherblinksCheckoutError } from "../src/errors";
...
describe("resolveInterval", () => {
  ...
  it("throws on invalid preset", () => {
    expect(() => resolveInterval("daily" as any)).toThrow(
      EtherblinksCheckoutError,
    );
  });
});
...
describe("createCheckoutUrl", () => {
  ...
  it("builds a valid checkout URL", () => {
    ...
    expect(parsed.origin).toBe("https://etherblinks.com");
    ...
    expect(parsed.searchParams.get("metadata_url")).toBe(
      validOptions.metadataUrl,
    );
    expect(parsed.searchParams.get("success_url")).toBe(
      validOptions.successUrl,
    );
  ...
  it("uses custom base URL", () => {
    const url = createCheckoutUrl({
      ...validOptions,
      baseUrl: "https://staging.autopay.xyz",
    });
  ...
  it("throws on invalid merchant address", () => {
    expect(() =>
      createCheckoutUrl({ ...validOptions, merchant: "not-an-address" }),
    ).toThrow(EtherblinksCheckoutError);
  });

  it("throws on invalid amount", () => {
    expect(() => createCheckoutUrl({ ...validOptions, amount: -1 })).toThrow(
      EtherblinksCheckoutError,
    );
    expect(() => createCheckoutUrl({ ...validOptions, amount: 0 })).toThrow(
      EtherblinksCheckoutError,
    );
  });

  it("throws on interval out of range", () => {
    expect(() => createCheckoutUrl({ ...validOptions, interval: 10 })).toThrow(
      EtherblinksCheckoutError,
    );
    expect(() =>
      createCheckoutUrl({ ...validOptions, interval: 365 * 86400 + 1 }),
    ).toThrow(EtherblinksCheckoutError);
  });

  it("throws on invalid URLs", () => {
    expect(() =>
      createCheckoutUrl({ ...validOptions, metadataUrl: "not-a-url" }),
    ).toThrow(EtherblinksCheckoutError);
    expect(() =>
      createCheckoutUrl({ ...validOptions, successUrl: "not-a-url" }),
    ).toThrow(EtherblinksCheckoutError);
    expect(() =>
      createCheckoutUrl({ ...validOptions, cancelUrl: "not-a-url" }),
    ).toThrow(EtherblinksCheckoutError);
  });

  it("throws when spending cap < amount", () => {
    expect(() =>
      createCheckoutUrl({ ...validOptions, spendingCap: 5 }),
    ).toThrow(EtherblinksCheckoutError);
  });
});
...
describe("parseSuccessRedirect", () => {
  ...
  it("throws on missing policyId", () => {
    expect(() => parseSuccessRedirect("?txHash=0xdef456")).toThrow(
      EtherblinksCheckoutError,
    );
  });

  it("throws on missing txHash", () => {
    expect(() => parseSuccessRedirect("?policyId=0xabc123")).toThrow(
      EtherblinksCheckoutError,
    );
  });
});
