...
import { EtherblinksWebhookError } from "../src/errors";
...
describe("verifyWebhook", () => {
  ...
  it("verifies and parses charge.failed", () => {
    const payload = makePayload("charge.failed", {
  ...
  it("throws on missing signature", () => {
    ...
    expect(() => verifyWebhook(payload, undefined, SECRET)).toThrow(
      EtherblinksWebhookError,
    );
  });

  it("throws on invalid signature", () => {
    const payload = makePayload("charge.succeeded");
    expect(() => verifyWebhook(payload, "wrong", SECRET)).toThrow(
      EtherblinksWebhookError,
    );
  });
  ...
  it("throws on missing secret", () => {
    const payload = makePayload("charge.succeeded");
    const sig = signPayload(payload, SECRET);
    expect(() => verifyWebhook(payload, sig, "")).toThrow(
      EtherblinksWebhookError,
    );
  });
  ...
  it("throws on unknown event type", () => {
    ...
    const sig = signPayload(payload, SECRET);
    expect(() => verifyWebhook(payload, sig, SECRET)).toThrow(
      EtherblinksWebhookError,
    );
  });
