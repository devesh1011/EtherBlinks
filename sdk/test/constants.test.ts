...
describe("chains", () => {
  it("has etherlink mainnet config", () => {
    expect(chains.etherlinkMainnet.chainId).toBe(42793);
    expect(chains.etherlinkMainnet.explorer).toBe(
      "https://explorer.etherlink.com",
    );
  });

  it("has etherlink shadownet config", () => {
    expect(chains.etherlinkShadownet.chainId).toBe(127823);
    expect(chains.etherlinkShadownet.explorer).toBe(
