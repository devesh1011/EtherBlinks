...
/**
 * Validate a metadata JSON object against the EtherBlinks metadata schema.
 *
 * @example
 * ```ts
...
export function validateMetadata(data: unknown): MetadataValidationResult {
  ...
  if (!obj.plan || typeof obj.plan !== "object") {
    ...
    if (typeof plan.name !== "string" || !plan.name) {
      errors.push(
        'Missing or invalid "plan.name" (must be a non-empty string)',
      );
    }
    if (typeof plan.description !== "string" || !plan.description) {
      errors.push(
        'Missing or invalid "plan.description" (must be a non-empty string)',
      );
    }
  ...
  if (!obj.merchant || typeof obj.merchant !== "object") {
    ...
    if (typeof merchant.name !== "string" || !merchant.name) {
      errors.push(
        'Missing or invalid "merchant.name" (must be a non-empty string)',
      );
    }
    ...
    if (
      ...
      typeof merchant.website !== "string"
    ) {
      errors.push('"merchant.website" must be a string if provided');
    }
    if (
      merchant.supportEmail !== undefined &&
...
export function createMetadata(options: {
  ...
  if (options.supportEmail)
    metadata.merchant.supportEmail = options.supportEmail;
