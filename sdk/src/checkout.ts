import { EtherblinksCheckoutError } from "./errors";
import {
  DEFAULT_CHECKOUT_BASE_URL,
  intervals as presetIntervals,
  MIN_INTERVAL,
  MAX_INTERVAL,
} from "./constants";
...
export function resolveInterval(interval: IntervalPreset | number): number {
  ...
  if (!seconds) {
    throw new EtherblinksCheckoutError(
...
export function createCheckoutUrl(options: CheckoutOptions): string {
  ...
  if (!merchant || !isValidAddress(merchant)) {
    throw new EtherblinksCheckoutError(`Invalid merchant address: ${merchant}`);
  }
  ...
  if (!isValidUrl(metadataUrl)) {
    throw new EtherblinksCheckoutError(`Invalid metadata URL: ${metadataUrl}`);
  }
  if (!isValidUrl(successUrl)) {
    throw new EtherblinksCheckoutError(`Invalid success URL: ${successUrl}`);
  }
  if (!isValidUrl(cancelUrl)) {
    throw new EtherblinksCheckoutError(`Invalid cancel URL: ${cancelUrl}`);
  }

  // Validate spending cap
  if (spendingCap !== undefined) {
    if (
      typeof spendingCap !== "number" ||
      spendingCap <= 0 ||
      !Number.isFinite(spendingCap)
    ) {
      throw new EtherblinksCheckoutError(
        `Invalid spending cap: ${spendingCap}. Must be a positive number.`,
      );
    }
    if (spendingCap < amount) {
      throw new EtherblinksCheckoutError(
        `Spending cap (${spendingCap}) must be >= amount (${amount}).`,
      );
    }
  }
...
export function parseSuccessRedirect(queryString: string): SuccessRedirect {
  ...
  if (!policyId) {
    throw new EtherblinksCheckoutError(
      "Missing policyId in success redirect URL",
    );
  }
  if (!txHash) {
    throw new EtherblinksCheckoutError(
      "Missing txHash in success redirect URL",
    );
  }
