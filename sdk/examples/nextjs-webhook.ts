 ...
 *
 * This example shows how to handle EtherBlinks webhooks in a Next.js API route.
 * Since Next.js App Router gives you the raw Request, you can use verifyWebhook directly.
 */
import { verifyWebhook } from "../src";

const WEBHOOK_SECRET =
  process.env.ETHERBLINKS_WEBHOOK_SECRET ?? process.env.AUTOPAY_WEBHOOK_SECRET!;

export async function POST(request: Request) {
  // Get raw body text for signature verification
  const rawBody = await request.text();
  const signature = request.headers.get("x-etherblinks-signature") ?? undefined;
  ...
  try {
    ...
    switch (event.type) {
      ...
      case "policy.cancelled_by_failure": {
        ...
        console.log(
          `Auto-cancelled for ${payer} after ${consecutiveFailures} failures`,
        );
