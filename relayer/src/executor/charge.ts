...
import SubscriptionManagerAbi from "../abis/PolicyManager.json" with { type: "json" };
import type {
  RelayerConfig,
  ChainConfig,
  ChargeResult,
...
type AbiType = typeof SubscriptionManagerAbi;
...
export async function canCharge(
  ...
  try {
    const result = (await publicClient.readContract({
      ...
      abi: SubscriptionManagerAbi as AbiType,
...
export async function chargePolicy(
  ...
  if (!canChargeResult.canCharge) {
    ...
    const isBalanceOrAllowanceIssue =
      canChargeResult.reason.includes("Insufficient");
    if (isBalanceOrAllowanceIssue) {
      logger.info(
        { policyId, reason: canChargeResult.reason },
        "Pre-check soft-fail (balance/allowance)",
      );
      return {
        success: false,
        softFailed: true,
  ...
  try {
    ...
    if (chargeFailedLog && !chargeSucceededLog) {
      logger.info(
        { policyId, txHash: hash },
        "Charge soft-failed (insufficient balance/allowance)",
      );
      return {
        success: false,
        softFailed: true,
...
export async function cancelFailedPolicyOnChain(
  ...
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Unknown error";
    logger.error(
      { policyId, error: errorMessage },
      "cancelFailedPolicy failed",
    );
...
function getEventSignature(eventName: string): `0x${string}` {
  ...
  if (eventName === "ChargeFailed") {
    return keccak256(toHex("ChargeFailed(bytes32,string)"));
  }
