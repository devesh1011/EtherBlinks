...
import SubscriptionManagerArtifact from "../abis/PolicyManager.json" with { type: "json" };
import type {
  PolicyCreatedEvent,
  PolicyRevokedEvent,
...
const SubscriptionManagerAbi = (SubscriptionManagerArtifact as any).abi;
...
export function parsePolicyCreated(log: Log): PolicyCreatedEvent | null {
  try {
    ...
    console.log('✅ Parsed PolicyCreated:', {
    ...
    console.error('❌ Failed to parse PolicyCreated:', {
...
export function parsePolicyCancelledByFailure(
  log: Log,
): PolicyCancelledByFailureEvent | null {
  try {
    const decoded = decodeEventLog({
      abi: SubscriptionManagerAbi as AbiType,
      data: log.data,
...
export function parseLog(log: Log): ParsedEvent | null {
  ...
  if (chargeSucceeded)
    return { type: "ChargeSucceeded", event: chargeSucceeded };
  ...
  if (policyCancelled)
    return { type: "PolicyCancelledByFailure", event: policyCancelled };
