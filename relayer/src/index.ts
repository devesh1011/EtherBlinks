...
export async function startRelayer() {
  logger.info("Starting EtherBlinks Subscriptions relayer...");
  ...
  if (config.merchantAddresses) {
    logger.info(
      {
        merchants: Array.from(config.merchantAddresses),
        count: config.merchantAddresses.size,
      },
