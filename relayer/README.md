...
│   │
│   ├── abis/
│   │   └── SubscriptionManager.json   # Contract ABI (copied from frontend)
│   │
│   ├── db/
...
```typescript
// Header: x-etherblinks-signature
...
│  │ (10s timeout)   │     │ Headers: x-etherblinks-signature      │           │
│  └────────┬────────┘     └──────────────────────────────────┘           │
│           │                                                              │
│     ┌─────┴─────┐                                                       │
│     │           │                                                       │
...
The relayer uses PostgreSQL. You have two options:

**Option A: Supabase (Recommended - Production Ready)**

The EtherBlinks Supabase project is already set up with all tables and indexes.

1. Get your database password from [Supabase Dashboard](https://supabase.com/dashboard/project/lhjentafnuthpkonwxkq/settings/database)
2. Update `.env` with:
   ```bash
   DATABASE_URL=postgres://postgres:[YOUR_PASSWORD]@db.lhjentafnuthpkonwxkq.supabase.co:5432/postgres
   ```
...
```bash
DATABASE_URL=postgres://etherblinks:password@localhost:5432/etherblinks
```

### 3. Run Migrations (Only needed for local database)

```bash
bun run cli -- db:migrate
```

> **Note:** If using Supabase, the database schema is already set up. Skip this step.
...
**SubscriptionManager** at `0xCa974B1EeC022B6E27bfA24D021F518C4d5b3734`
