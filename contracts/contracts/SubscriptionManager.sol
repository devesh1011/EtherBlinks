...
contract SubscriptionManager is ReentrancyGuard, Ownable {
    ...
    function createPolicy(
        ...
        if (USDC.allowance(msg.sender, address(this)) < chargeAmount)
            revert InsufficientAllowance();
        if (USDC.balanceOf(msg.sender) < chargeAmount)
    ...
    function charge(
        ...
        if (!policy.active) revert PolicyNotActive();
        if (policy.consecutiveFailures >= MAX_RETRIES)
            revert MaxRetriesReached();
        if (block.timestamp < policy.lastCharged + policy.interval) {
            revert TooSoonToCharge();
        }
        if (
            policy.spendingCap > 0 &&
            policy.totalSpent + policy.chargeAmount > policy.spendingCap
        ) {
    ...
    function getChargeBreakdown(
        bytes32 policyId
    )
        external
        view
        returns (uint256 total, uint256 merchantReceives, uint256 protocolFee)
    {
        Policy storage policy = policies[policyId];
